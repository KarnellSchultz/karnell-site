...
The useEffect is only there to `console.log` the value of the
`inputRef` value. its only there for demonstration purposes and so we
have something to look at in the log. Want to learn more about
useEffect?
